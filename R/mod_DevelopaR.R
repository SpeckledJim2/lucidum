...
mod_DevelopaR_ui <- function(id){
  ...
  tagList(
    tabsetPanel(id = ns('tabsetPanel'),
                tabPanel(value = 'KPI specification', title = span(tagList(icon('gears'), 'KPI specification')),
                         br(),
                         fluidRow(
                           column(
                             width = 4,
...
#' 
mod_DevelopaR_server <- function(id, d, dt_update, kpi_spec, filter_spec, feature_spec){
